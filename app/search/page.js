  ...
  return (
    <main className="min-h-screen" style={{ background: "#F5F3FF" }}>
      ...
      <div style={{ maxWidth: "800px", margin: "0 auto", padding: "32px 24px" }}>
        ...
        {results.length > 0 && (
          <div>
            <p style={{ fontSize: "13px", color: "#6366F1", fontWeight: "500", marginBottom: "14px" }}>{results.length} result{results.length > 1 ? "s" : ""} for "{query}"</p>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {results.map((tool) => (
                <ToolCard key={tool.href} tool={tool} query={query} />
              ))}
            </div>
          </div>
        )}
    ...
    <Link href={tool.href} style={{ background: "white", border: "0.5px solid #E0E7FF", borderRadius: "12px", padding: "16px 20px", textDecoration: "none", display: "flex", alignItems: "center", gap: "14px" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "3px" }}>
          ...
          {tool.badge && <span style={{ fontSize: "10px", padding: "2px 7px", borderRadius: "4px", background: "#EEF2FF", color: "#4F46E5", fontWeight: "500" }}>{tool.badge}</span>}
        </div>
